"""
seed_data.py - Seed the elections database with 37 states, 774 LGAs, and political parties.
"""
from models import db, State, LGA, Party
...
PARTIES_DATA = [
    ("All Progressives Congress", "APC", "#009401"),
    ("Peoples Democratic Party", "PDP", "#E3242B"),
    ("African Democratic Congress", "ADC", "#0066B3"),
    ("Labour Party", "LP", "#FF6B00"),
    ("Social Democratic Party", "SDP", "#FFD700"),
    ("All Progressives Grand Alliance", "APGA", "#800080"),
    ("New Nigeria Peoples Party", "NNPP", "#DC143C"),
    ("Zenith Labour Party", "ZLP", "#228B22"),
    ("Young Progressives Party", "YPP", "#FF1493"),
    ("Accord Party", "Accord", "#4169E1"),
    ("Boot Party", "BOOT", "#8B4513"),
    ("Action Alliance", "AA", "#696969"),
    ("Action Democratic Party", "ADP", "#2F4F4F"),
    ("African Action Congress", "AAC", "#FF4500"),
    ("Action Peoples Party", "APP", "#A0522D"),
    ("Allied Peoples Movement", "APM", "#6B8E23"),
...
def seed():
    """Seed states, LGAs, and parties into the database."""
    ...
    for state_name, lga_names in LGAS_DATA.items():
        ...
        for lga_name in lga_names:
            ...
            total_lgas += 1
    db.session.flush()
    print(f"  -> {total_lgas} LGAs created.")

    print("Seeding parties...")
    for full_name, abbr, color in PARTIES_DATA:
        party = Party(name=full_name, abbreviation=abbr, color=color)
        db.session.add(party)
    db.session.flush()
    print(f"  -> {len(PARTIES_DATA)} parties created.")
